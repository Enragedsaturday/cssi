...
```mermaid
flowchart TD
  A[Warnings given] --> B{Suspect's response}
  B -->|Waive: express or implied / Thompkins| C[Questioning OK]
  B -->|Invoke counsel: must be unambiguous / Davis| D[Edwards bar: no police re-initiation]
  D --> E[Counsel present / Minnick<br/>OR break in custody + 14 days / Shatzer<br/>continuous custody is NOT a break / Wint]
  E --> F[May seek fresh waiver]
  B -->|Invoke silence: must be unambiguous / Thompkins| G[Scrupulously honor / Mosley]
```
