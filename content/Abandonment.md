...
| *Jones v. United States*, 362 U.S. 257 (1960) | Overruled "automatic standing" (by *Salvucci*) — relevant to abandonment because the rule that once shielded possessors who disclaimed property is gone, so disclaiming a connection to an item now defeats, not preserves, the ability to object. | [[Standing to Challenge a Search]] | [opinion](https://www.courtlistener.com/opinion/106022/jones-v-united-states/) |
...
```mermaid
flowchart TD
  Q{"Did the person retain a reasonable<br/>expectation of privacy?<br/><i>(denial · discard · location · intent)</i>"}
  Q -->|No| AB["Abandoned → no standing<br/>Greenwood · Abel · Hester"]
  Q -->|Yes| BAIL["REP retained<br/>(e.g. bailment · Bond)"]
  BAIL --> C{"Consent by one with<br/>common authority? (Matlock)"}
  C -->|Yes| OK["Search may proceed"]
  C -->|No| WAR["Need warrant or exception"]
```
