...
```mermaid
flowchart TD
    A[ID challenge] --> B{Which doctrine?}
    B -->|6A counsel| C{Post-charge?}
    C -->|No: pre-charge| D[No right — Kirby]
    C -->|Yes| E{Corporeal lineup?}
    E -->|No: photo array| F[No right — Ash]
    E -->|Yes| G[Right to counsel — Wade/Gilbert]
    B -->|Due process| H{Police arranged?}
    H -->|No| I[No screen — Perry]
    H -->|Yes| J{Unnecessarily suggestive?}
    J -->|No| K[Admissible]
    J -->|Yes| L{Reliable? Biggers/Manson factors}
    L -->|Yes| K
    L -->|No| M[Suppress]
```
