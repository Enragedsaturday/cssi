...
```mermaid
flowchart TD
  A["Tent used as a dwelling<br/>Apply Katz: subjective + objective REP"] --> B{Subjective signs?<br/>closed sides · not visible in · effects left inside}
  B -->|No| NP["No protected expectation"]
  B -->|Yes| C{Right to be there?}
  C -->|"Camper — private land,<br/>campground, or public/BLM land<br/>(permission NOT required)"| R["REP in the tent<br/>Gooch · Sandoval"]
  C -->|"Trespasser/squatter,<br/>no colorable right<br/>(esp. fixed structure)"| NR["REP may fail<br/>Ruckman (10th Cir.) ⚖"]
  R --> D["Interior = protected<br/>warrant or exception required"]
  R --> E["Open area around tent<br/>= NOT curtilage · Basher"]
```
