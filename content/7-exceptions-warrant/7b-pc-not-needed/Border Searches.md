...
```mermaid
flowchart TD
  A["Search at the border<br/>or its functional equivalent?"] -->|"No — deep interior<br/>(roving patrol)"| Z["Border exception N/A:<br/>need PC, warrant, or consent<br/>(Almeida-Sanchez)"]
  A -->|Yes| B{"Routine, or<br/>non-routine?"}
  B -->|"Routine: persons, baggage,<br/>property — incl. destructive<br/>(gas-tank disassembly · Flores-Montano)"| C["NO warrant / PC / suspicion<br/>(Ramsey)"]
  ...
  E -->|"9th (Cotterman/Cano) · 4th (Kolsuz/Aigbekaen,<br/>+ border nexus) · 1st (Alasaad, advanced)"| F["Reasonable suspicion"]
  E -->|"11th (Touset) — devices are property"| G["No suspicion"]
  E -->|"Manual device search<br/>(broad consensus)"| H["No suspicion — routine"]
```
