...
```mermaid
flowchart TD
  A[Warrant application] --> B{Probable cause?}
  B -- No --> X[Invalid]
  B -- Yes --> C{Oath / affirmation?}
  C -- No --> X
  C -- Yes --> D{Neutral & detached magistrate?}
  D -- No --> X
  D -- Yes --> E{Particular: place + things?}
  E -- No --> X
  E -- Yes --> V[Valid warrant]
  X -.Franks falsehood.-> F[Franks]
