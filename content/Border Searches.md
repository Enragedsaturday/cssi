...
```mermaid
flowchart TD
  A[Search at border / functional equivalent?] -->|No, deep interior| Z[Border exception N/A: PC, warrant, or consent]
  A -->|Yes| B{Routine or non-routine?}
  B -->|Routine: persons, baggage, property incl. destructive| C[No warrant / PC / suspicion]
  B -->|Non-routine: prolonged detention, body cavity| D[Reasonable suspicion]
  B -->|Forensic device search| E[Unresolved split]
  E --> F[9th Cir.: reasonable suspicion]
  E --> G[11th Cir.: no suspicion]
```
